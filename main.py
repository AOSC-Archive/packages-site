...
@app.route('/depgraph/<name>')
def dep_graph(name, db):
    try:
        graph = depgraph.pkgdep_graph(name, db, False)
    except KeyError:
        return bottle.HTTPResponse(render('error.html',
                error='Package "%s" not found.' % name), 404)
...
@app.route('/revdepgraph/<name>')
def revdep_graph(name, db):
    try:
        graph = depgraph.pkgdep_graph(name, db, True)
    except KeyError:
        return bottle.HTTPResponse(render('error.html',
                error='Package "%s" not found.' % name), 404)
    bottle.response.content_type = 'image/svg+xml; charset=UTF-8'
